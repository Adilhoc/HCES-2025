...
def context_row(base):
    ...
    return {
        ...
        "weight": num(base.get("Multiplier")),
        "household_size": 0,
...
def process_food(contexts):
    ...
    for row in read_csv(level5):
        ...
        value = num(row.get("Total_Consumption_Value"))
        quantity = num(row.get("Total_Consumption_Quantity"))
        out_value = num(row.get("OutOfHome_Consumption_Value"))
        out_qty = num(row.get("OutOfHome_Consumption_Quantity"))
...
def write_cube(cube):
    ...
    with gzip.open(out_path, "wt", newline="", encoding="utf-8") as fh:
        ...
        for key, stats in cube.items():
            row = dict(zip(fields[:14], key))
            for stat_key in fields[14:18]:
