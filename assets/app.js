...
function sourceFilterActive() {
  return selectedValue('sourceSelect') !== 'All';
}
...
function itemMatches(row, options = {}) {
  ...
  return (!states.length || states.includes(row.state)) &&
    ...
    (ration === 'All' || row.ration_any === ration) &&
    (pds === 'All' || row.pds_food === pds) &&
    (online === 'All' || row.online_food === online) &&
    (ceremony === 'All' || row.ceremony === ceremony) &&
    (sizeBand === 'All' || row.household_size_band === sizeBand) &&
    (expBand === 'All' || row.monthly_exp_band === expBand) &&
    (ignoreCategory || category === 'All' || row.category_slug === category);
}

function hasPdsFood(row) {
  if (row.pds_food === 'Yes' || row.pds_food === 'No') return row.pds_food;
...
function getFilteredItems(options = {}) {
  return ITEM_CUBE.filter(row => itemMatches(row, options));
}
...
function weightedOutHomeFromItems(rows) {
  return rows.reduce((total, row) => total + num(row.weighted_out_home_value), 0);
}
...
function currentOutHomeValue(stats, items) {
  if (sourceFilterActive()) return weightedOutHomeFromItems(items);
  return stats.outHome;
}
...
function updateDashboard() {
  ...
  const outHomeValue = currentOutHomeValue(stats, items);
  const currentMetric = metricFromTotals(metric, totalValue, stats.weight, stats.persons, outHomeValue);
...
function householdCategoryValues(rows) {
  return META.categories.map(category => {
    let total = 0;
    rows.forEach(row => {
      total += num(row[category.column]) * num(row.weight);
    });
    return { slug: category.slug, name: category.label, value: total };
  });
}

function itemCategoryValues(rows) {
  const labelsBySlug = new Map(META.categories.map(category => [category.slug, category.label]));
  return groupRows(
    rows,
    row => row.category_slug || 'other',
    row => ({
      slug: row.category_slug || 'other',
      name: labelsBySlug.get(row.category_slug) || row.category || 'Other food',
      value: 0
    }),
...
function selectedVsOtherCategory(values, category) {
  if (!category) return values;
  const selected = values.find(row => row.slug === category.slug);
  const otherValue = values.reduce((total, row) => row.slug === category.slug ? total : total + row.value, 0);
  return [
    { slug: category.slug, name: category.label, value: selected ? selected.value : 0 },
    { slug: 'other_categories', name: 'Other food categories', value: otherValue }
  ];
}

function renderCategoryChart(households, items) {
  const category = categoryMeta();
  const sourceAwareItems = sourceFilterActive()
    ? (category ? getFilteredItems({ ignoreCategory: true }) : items)
    : null;
  const rawValues = sourceAwareItems ? itemCategoryValues(sourceAwareItems) : householdCategoryValues(households);
  const values = selectedVsOtherCategory(rawValues, category)
    .filter(row => row.value > 0)
    .sort((a, b) => b.value - a.value);

  const scopeLabel = sourceAwareItems
    ? 'Source-aware item values'
    : 'Household category subtotals';
  document.getElementById('categoryScopeLabel').textContent = category
    ? `${category.label} vs other food categories`
    : scopeLabel;
...
function stateSummary(households, items, metric, category) {
  ...
  if (sourceFiltered) {
    ...
    items.forEach(row => {
      ...
      if (den) {
        den.value += num(row.weighted_value);
        den.outHome += num(row.weighted_out_home_value);
      }
    });
  }
...
function itemSpendGroups(items) {
  return groupRows(
    items,
    row => `${row.item_code}|${row.item_name}`,
    row => ({ code: row.item_code, name: row.item_name, value: 0, outHome: 0 }),
    (group, row) => {
      group.value += num(row.weighted_value);
      group.outHome += num(row.weighted_out_home_value);
    }
  ).filter(row => row.value > 0).sort((a, b) => b.value - a.value);
}
...
function renderItemSpendChart(items, stats) {
  const topN = Math.max(5, Math.min(12, Number(selectedValue('topN') || 12)));
  const grouped = itemSpendGroups(items);

  if (!grouped.length) return emptyPlot('itemSpendChart', 'No matching item rows');

  const topRows = grouped.slice(0, topN);
  const otherValue = grouped.slice(topN).reduce((total, row) => total + row.value, 0);
  const values = otherValue > 0
    ? [...topRows, { code: '', name: 'Other items', value: otherValue, outHome: 0 }]
    : topRows;
  const householdWeight = stats.weight || 0;

  Plotly.react('itemSpendChart', [{
    type: 'pie',
    labels: values.map(itemLabel),
    ...
    customdata: values.map(row => [
      householdWeight ? inr(row.value / householdWeight) : 'Rs.0',
      inrCompact(row.value)
    ]),
    hovertemplate: '%{label}<br>%{percent}<br>Avg household value: %{customdata[0]}<br>Total weighted value: %{customdata[1]}<extra></extra>'
  }], {
    ...plotLayoutBase,
    margin: { l: 8, r: 8, t: 8, b: 8 },
    showlegend: true,
...
function renderItemChart(items) {
  const topN = Math.max(5, Math.min(30, Number(selectedValue('topN') || 12)));
  const grouped = itemSpendGroups(items).slice(0, topN).reverse();
...
function populateControls() {
  ...
  fillSelect('sizeBandSelect', ['1', '2', '3 to 4', '5 to 6', '7+', 'Unknown']);
