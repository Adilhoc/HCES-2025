...
async function loadCompressedData(){
  ...
  try {
    ...
    if (!response.ok) { throw new Error(`HTTP ${response.status} while fetching data/health_level5_slim.csv.gz`); }
    ...
    Papa.parse(csvText, {
      ...
      error: function(err){ status.textContent = 'Could not parse data: ' + err; }
    });
  } catch (err) {
