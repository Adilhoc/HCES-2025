...
function renderMap(rows, metric) {
  ...
  const sizes = s.map(d => Math.max(9, Math.sqrt(d.weight / maxWeight) * 44));
  ...
  Plotly.react('mapChart', [{
    ...
    marker: {
      size: sizes,
      color: values,
      colorscale: [[0, '#064e3b'], [.5, '#fbbf24'], [1, '#10b981']],
      colorbar: { title: metricDefs[metric].label, thickness: 12, tickfont: { color: '#7a8a9e' }, titlefont: { color: '#c5d0dc' } },
